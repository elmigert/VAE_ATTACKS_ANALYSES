...
def main():
    ...
    parser.add_argument("--adv", help="what single adversarial to choose for minimax?",
                        choices = ['PGD', 'FGSM', 'GTA','GNA', 'VAEA','LatentA','Graphic','RANDOM'], default='FGSM')
    ...
    init = empty()
    init.adv = ['FGSM','PGD','GTA','GNA','VAEA','LatentA']
        ...
        # forward pass
                ...
                for idx, (x, y) in enumerate(train_loader):
                    ...
                    # create adv images.perform step on the reconstructed image generated by adv images.
                        ...
                        if args.adv == "FGSM" :
                            ...
                            #print('GNA Attack')
                            adv = A.GNA(x,  clip_min=args.clip_min, clip_max=args.clip_max,_sigma=0.0,_mean=0.0,rand=True)
                            #Test code for picture
                            locN = 0
                            pic_name = f'GNA{exp_name}'
                            if random.random()<0.02:
                                while os.path.exists(f'/content/drive/MyDrive/test_folder/{pic_name}{locN}.png'):
                                    locN = locN + 1
                                save_image(adv.view(adv.size(0), 1, 28, 28).data.cpu(),
                                           f'/content/drive/MyDrive/test_folder/{pic_name}{locN}.png')

                        elif args.adv == "GTA":
                            #print('GNA Attack')
                            #adv = A.GTA(x,rotate=30,translate=([0.01,0.01]),scale = ([0.8,1.2]),rand=True)
                            adv = A.GTA(x,rotate=0,translate=([0.00,0.00]),scale = ([1,1]),rand=True)
                        elif args.adv == "VAEA":
                            ...
                            adv = A.VAEA(x, target, chosen_norm, steps=10, eps=0.08, eps_norm=chosen_norm)
            ...
            for idx, (images, _) in enumerate(train_loader):
                ...
                if t >= learning_starts and t % train_freq == 0:
                    ...
                    if args.prioritized_replay:
                        experience = replay_buffer.sample(args.bs, beta=beta_schedule.value(t))
            ...
            def train_critic(critic, xs, losses, is_fc, att_type, losses_new =None):
                pred_losses = critic.pred(xs,att_type)
                real_loss = torch.nn.MSELoss(pred_losses,losses)
            ...
            for idx, (images, _) in enumerate(train_loader):
                ...
                optimizer.zero_grad()  # this might need to change.
                if args.use_critic:
                    train_critic(critic,images,losses,is_fc,np.array([args.model]*args.bs))
                loss = torch.sum(losses)
                loss.backward()
                optimizer.step()
                train_loss += loss.item()
                ...
                if t >= learning_starts and t % train_freq == 0:
                    ...
                    if args.use_critic:
                        batch_idxes=batch_idxes[new_idxes]
                    if args.prioritized_replay:
                        new_priorities = np.abs(td_errors.detach().numpy()) + prioritized_replay_eps
                        # print(new_priorities.shape)
                        replay_buffer.update_priorities(batch_idxes, new_priorities,losses_new.detach().numpy())
